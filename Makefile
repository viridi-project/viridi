...
pkgdatadir = $(datadir)/viridi
pkgincludedir = $(includedir)/viridi
pkglibdir = $(libdir)/viridi
pkglibexecdir = $(libexecdir)/viridi
...
CONFIG_HEADER = $(top_builddir)/src/config/viridi-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/viridi-config.h.in \
...
ACLOCAL = ${SHELL} /root/viridicoinx/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/viridicoinx/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/viridicoinx/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/viridicoinx/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = viridi-cli
BITCOIN_DAEMON_NAME = viridid
BITCOIN_GUI_NAME = viridi-qt
BITCOIN_TX_NAME = viridi-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/viridicoinx/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = viridi
PACKAGE_BUGREPORT = www.viridicoin.info
PACKAGE_NAME = VIRIDI Core
PACKAGE_STRING = VIRIDI Core 1.0.2
PACKAGE_TARNAME = viridi
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/viridicoinx
abs_srcdir = /root/viridicoinx
abs_top_builddir = /root/viridicoinx
abs_top_srcdir = /root/viridicoinx
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/viridicoinx/build-aux/install-sh
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/viridi.icns
OSX_PLIST = $(top_srcdir)/share/qt/Info.plist #not installed
OSX_QT_TRANSLATIONS = da,de,es,hu,ru,uk,zh_CN,zh_TW
DIST_DOCS = $(wildcard doc/*.md) $(wildcard doc/release-notes/*.md)
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/viridi.ico \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_viridi_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_viridi_coverage.info test_viridi.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/viridi.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/VIRIDI-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/viridi-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/viridi-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/viridi-config.h
$(top_srcdir)/src/config/viridi-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/viridi-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/Resources/viridi.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/VIRIDI-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/VIRIDI-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/VIRIDI-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_viridi.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_viridi -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_viridi_filtered.info: test_viridi.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_viridi_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_viridi_coverage.info: baseline_filtered_combined.info test_viridi_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_viridi_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_viridi_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_viridi_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_viridi.coverage/.dirstamp:  test_viridi_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_viridi.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local:
	rm -rf test_viridi.coverage/ total.coverage/ $(OSX_APP)
